...
module "viz-step-functions" {
  ...
  ripple_fim_data_prep_arn = lookup(var.deploy_resources, "ripple-fim-data-prep", false) ? module.viz-lambda-functions.ripple_fim_data_prep.arn : var.lambda_functions["ripple-fim-data-prep"].arn
  ripple_fim_processing_arn = lookup(var.deploy_resources, "ripple-fim-processing", false) ? module.viz-lambda-functions.ripple_fim_processing.arn : var.lambda_functions["ripple-fim-processing"].arn
