...
INTO publish.srf_18hr_max_inundation_building_footprints
FROM external.building_footprints_fema as buildings
JOIN fim_ingest.srf_18hr_max_inundation_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, buildings.geom)
...
--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.srf_18hr_max_inundation_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.srf_18hr_max_inundation_counties
...
-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.srf_18hr_max_inundation_hucs;
SELECT
	hucs.huc10,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.srf_18hr_max_inundation_hucs
