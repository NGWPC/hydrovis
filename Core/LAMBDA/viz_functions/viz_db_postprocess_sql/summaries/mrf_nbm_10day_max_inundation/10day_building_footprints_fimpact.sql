...
JOIN fim_ingest.mrf_nbm_max_inundation_10day_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, buildings.geom)
JOIN fim_ingest.mrf_nbm_max_inundation_10day_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.mrf_nbm_max_inundation_10day fim ON fim_geo.hand_id = fim.hand_id;
...
DROP TABLE IF EXISTS publish.mrf_nbm_10day_max_inundation_critical_infrastructure;
...
SELECT
	...
	flows.discharge_cfs AS streamflow_cfs,
	fim.rc_stage_ft AS fim_stage_ft
INTO publish.mrf_nbm_10day_max_inundation_critical_infrastructure
FROM derived.static_public_critical_infrastructure_points_fema as critpoints
JOIN fim_ingest.mrf_nbm_max_inundation_10day_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, critpoints.geometry)
JOIN fim_ingest.mrf_nbm_max_inundation_10day_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.mrf_nbm_max_inundation_10day fim ON fim_geo.hand_id = fim.hand_id;