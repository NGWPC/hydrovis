...
JOIN fim_ingest.mrf_nbm_max_inundation_3day_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, buildings.geom)
JOIN fim_ingest.mrf_nbm_max_inundation_3day_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.mrf_nbm_max_inundation_3day fim ON fim_geo.hand_id = fim.hand_id;
...
DROP TABLE IF EXISTS publish.mrf_nbm_3day_max_inundation_critical_infrastructure;
...
CREATE INDEX IF NOT EXISTS idx_fim_geo_geom ON fim_ingest.mrf_nbm_max_inundation_3day_geo USING GIST(geom);
ANALYZE fim_ingest.mrf_nbm_max_inundation_3day_geo;
SELECT
	critpoints.oid,
	...
	flows.discharge_cfs AS streamflow_cfs,
	fim.rc_stage_ft AS fim_stage_ft
INTO publish.mrf_nbm_3day_max_inundation_critical_infrastructure
FROM derived.static_public_critical_infrastructure_points_fema as critpoints
JOIN fim_ingest.mrf_nbm_max_inundation_3day_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, critpoints.geometry)
JOIN fim_ingest.mrf_nbm_max_inundation_3day_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.mrf_nbm_max_inundation_3day fim ON fim_geo.hand_id = fim.hand_id;