...
INTO publish.rfc_based_5day_max_inundation_building_footprints
FROM external.building_footprints_fema as buildings
JOIN fim_ingest.rfc_based_5day_max_inundation_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, buildings.geom)
JOIN fim_ingest.rfc_based_5day_max_inundation_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.rfc_based_5day_max_inundation fim ON fim_geo.hand_id = fim.hand_id;

--------------- County Summary ---------------
DROP TABLE IF EXISTS publish.rfc_based_5day_max_inundation_counties;
SELECT
	counties.geoid,
	...
	counties.geom
INTO publish.rfc_based_5day_max_inundation_counties
FROM derived.counties AS counties
JOIN derived.channels_county_crosswalk AS crosswalk ON counties.geoid = crosswalk.geoid
JOIN publish.rfc_based_5day_max_inundation AS fim on crosswalk.feature_id = fim.feature_id
JOIN publish.rfc_based_5day_max_inundation_building_footprints AS buildings ON crosswalk.feature_id = buildings.feature_id
GROUP BY counties.geoid, counties.name, counties.geom, buildings.prop_st;

-------------- HUCS Summary ---------------
DROP TABLE IF EXISTS publish.rfc_based_5day_max_inundation_hucs;
SELECT
	hucs.huc8,
	...
	to_char(now()::timestamp without time zone, 'YYYY-MM-DD HH24:MI:SS UTC') AS update_time,
	hucs.geom
INTO publish.rfc_based_5day_max_inundation_hucs
...
DROP TABLE IF EXISTS publish.rfc_based_5day_max_inundation_critical_infrastructure;
...
CREATE INDEX IF NOT EXISTS idx_fim_geo_geom ON fim_ingest.rfc_based_5day_max_inundation_geo USING GIST(geom);
ANALYZE fim_ingest.rfc_based_5day_max_inundation_geo;
SELECT
	critpoints.oid,
	...
	flows.discharge_cfs AS streamflow_cfs,
	fim.rc_stage_ft AS fim_stage_ft
INTO publish.rfc_based_5day_max_inundation_critical_infrastructure
FROM derived.static_public_critical_infrastructure_points_fema as critpoints
JOIN fim_ingest.rfc_based_5day_max_inundation_geo fim_geo ON ST_INTERSECTS(fim_geo.geom, critpoints.geometry)
JOIN fim_ingest.rfc_based_5day_max_inundation_flows flows ON fim_geo.hand_id = flows.hand_id
JOIN fim_ingest.rfc_based_5day_max_inundation fim ON fim_geo.hand_id = fim.hand_id;