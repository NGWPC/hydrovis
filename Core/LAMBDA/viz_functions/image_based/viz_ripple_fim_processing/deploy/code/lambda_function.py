...
def track_changes(
    ...
    try:
        ...
        print("[Step:3-2-1] Error uploading summary to tracking table")
...
#Example input from Step Function lambda call
#{'Bucket': 'hv-vpp-dev-ripple', 'FolderName': 'dev_temp/LorneLeonard/02-21-25/'}"
#02-22-25 Changed strategies using csv file with map function
#csv contains full path to base folder.
#"Input":"{\"Type\":\"MIP\",\"Model_FullPath\":\"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/02-21-25/mip_12010004/\"}",
    ...
    if argument_method_step == True:
        arg_input_from_map_json = ''
        arg_input_model_fullpath = ''
        ...
        try:
            ...
            input_maxFlowTable = event['maxFlowTable']

            #TODO come back and evaluate if it still makes sense to break model s3 into parts
            #BEWARE expecting / in database at start and end
            #TODO error checking
            ...
            print("[Step Function 2]")

            ################################
            #TODO
            # 03-07-25 I modified the map csv file structure to avoid having bucket info in code
            # like these tests
            ################################
            arg_input_maxFlowTable = event['maxFlowTable']
            if (console_debugging): 
                print(f"[Step:1] arg_input_maxFlowTable = {arg_input_maxFlowTable}")
#Type,maxFlowTable,Model_FullPath
#BLE,cache.max_flows_ana_14day,s3://hv-vpp-dev-ripple/input/ngwpc_data/30_pcnt_domain/ble_11140102_Blue/

#oid,model_bucket,model_key,model_name,maxFlowTable
#2,
# model_bucket  = s3://hv-vpp-dev-ripple,
# model_key     =  /input/ngwpc_data/30_pcnt_domain/,
# model_name    = mip_12010005/,
# maxFlowTable  =  s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/LAMBDA_WORKSPACE/cache.max_flows_srf.csv

            print("[Step Function 3]")
            #Error checking is done in Step2
            flow_file = arg_input_maxFlowTable

            #flow_file = "s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/flows_100year.csv"  #WORKED USED FOR TESTING
            #flow_file = f"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/03-06-25/{arg_input_maxFlowTable}.csv"  #TODO

            if (console_debugging): 
                print(f"[Step:1] flow_file = {flow_file}")

            #print("[Step Function 4]")
            #flow_file = "s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/flows_100year.csv"  #WORKED USED FOR TESTING

    # table_list.append("max_flows_ana")
    # table_list.append("max_flows_ana_7day")
    # table_list.append("max_flows_ana_14day")
    # table_list.append("max_flows_mrf_gfs_3day")
    # table_list.append("max_flows_mrf_gfs_5day")
    # table_list.append("max_flows_mrf_gfs_10day")
    # table_list.append("max_flows_srf")
            ################################
            ...
            #output_fim_raster_file = f"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/Lambda_Tests/STEP_TEST/STEP_fim{random_uuid}.tif"
            #output_vector_file = f"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/Lambda_Tests/STEP_TEST/STEP_fim_ble{random_uuid}.parquet"
            #create_both_outputs = "TRUE" #"FALSE" #Default value
            #output_control_file = f"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/Lambda_Tests/STEP_TEST/STEP_controls{random_uuid}.csv"
            #output_log_file = f"s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/Lambda_Tests/STEP_TEST/lamb_logs{random_uuid}.txt"
            ...
            env_db_database = os.getenv('VIZ_DB_DATABASE')  #vizprocessing
            env_db_schema = os.getenv('VIZ_DB_SCHEMA')      #dev
            env_db_tablename = os.getenv('VIZ_OUT_DB_TABLE_PREFIX')    #leonard_flow_test_lamb #TODO
            env_SRID = os.getenv('VIZ_SRID')                #3857

#TODO HACK for testing
            flow_file_bucket, flow_file_key, flow_file_fname = parse_s3_url_GET_bucket_key_filename(arg_input_maxFlowTable)
            tmp = str(flow_file_fname).replace("cache.","")
            tmp = str(tmp).replace(".csv","")
            #env_db_tablename = "leonard_ripple_" + str(tmp)
            env_db_tablename = env_db_tablename + str(tmp)
...
#            arg_input_maxFlowTable = event['maxFlowTable']
    ...
    ##############################################
            ...
            return {
                'message': f"[Step:0-1-0] Argument missing errors. {event} XXX {arg_input_from_map_json} XXX {arg_input_model_fullpath} XXX {traceback_str}"
            }
    ...
    if argument_method_step == False: #This means Lambda handle via Test i.e. Not a batch job
        ...
        if not check_environment_value(env_db_schema):
            return {
                'message': f"[Step:0-2-5] Invalid table schema."
            }
        if not check_environment_value(env_db_tablename):
            return {
                'message': f"[Step:0-2-6] Invalid table name."
            }
        if not check_environment_value(env_SRID):
            return {
                'message': f"[Step:0-2-7] Invalid SRID value."
            }
    ...
    logger.debug(f"[Step:2-0-02] {flow_file}")   #Step 3
    ...
    logger.debug(f"[Step:2-1-3] Download Ripple file {flow_file}")
    flow_bucket, flow_key, flow_fname = parse_s3_url_GET_bucket_key_filename(flow_file)
    if (console_debugging):
    ...
    ##############################################
            ...
            logger.critical(f"[Step:2-1-3] Something else went wrong with downloading '{flow_file}'")
            print(f"[Step:2-1-3] Something else went wrong with downloading '{flow_file}'")
    ...
    if os.path.exists(str(output_control_local_path)):
        ...
        if file_size < 30:
            #logger.critical(f"[Step:3-0-4] Control file size created too small. '{file_size}' -> '{output_control_local_path}'")
...
#######################################################################################################################
## 03-04-25 THIS WORKED ON THE FLY CONCEPT
#######################################################################################################################

#Next test is using a pre-computed parquet file
#s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/03-03-25/BatchTest1.parquet
    if False:
        print("Hack test using Control File and cached parquet file")

        s3_path_cached = "s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/03-03-25/BatchTest1.parquet"

        #Worked fine here
        #viz_engine = create_engine(f'postgresql://{env_db_username}:{env_db_password}@{env_db_host}/{env_db_database}')
        #df = pq.read_table(s3_path_cached)
        #df = df.to_pandas() #Doubles memory https://arrow.apache.org/docs/python/pandas.html

        #S3 Test
        #df = gpd.read_parquet(s3_path_cached) #Works just too much memory
        #KEEP WORKED
        #KEEP1 df = gpd.read_parquet(s3_path_cached)
        #df = dask_geopandas.read_parquet(s3_path_cached) #Getting permissions issues TODO
        ...
        ripple_local_pathX = '/tmp/BatchTest1.parquet'

        ripple_bucketX, ripple_keyX, ripple_fnameX = parse_s3_url_GET_bucket_key_filename(s3_path_cached)
        #s3.Bucket(ripple_bucketX).download_file(ripple_keyX, ripple_local_pathX)

        #Local file copy test
        #df = gpd.read_parquet(ripple_local_pathX)

#gpd.GeoDataFrameX

        logger.debug(f"[Step:4-0-1] Opening Control File {output_control_local_path}")
        with open(output_control_local_path, 'r') as c_file:
            ...
            for line in c_file:
                ...
                try:
                    ...
                    #master_gdf = df.query(f"FeatureID == {cf_reach_id} and Stage == {cf_control_stage} and Flows == {cf_flow}")
                    #KEEP master_gdf = df.query(f"FID == '{cf_reach_id}' and Stage == '{cf_control_stage}' and Extent == '{cf_flow}'")

                    #Works but takes too much memory
                    #KEEP1 master_gdf = df.query(f"FID == '{cf_reach_id}' and Stage == '{cf_control_stage}' and Extent == '{cf_flow}'")

                    #master_gdf = df.query(f"FID == '{cf_reach_id}' and Stage == '{cf_control_stage}' and Extent == '{cf_flow}'")
                    #query = f"FID == '{cf_reach_id}' and Stage == '{cf_control_stage}' and Extent == '{cf_flow}'"

                    query = f"""SELECT * FROM s3object WHERE s3object.FID == '{cf_reach_id}' AND s3object.Stage == '{cf_control_stage}' AND s3object.Extent == '{cf_flow}'"""

                    select_params = {
                        'Bucket': ripple_bucketX,
                        'Key': ripple_keyX,
                        'ExpressionType': 'SQL',
                        'Expression': query,
                    ...
                    if 'Payload' not in response:
                        ...
                        records = response['Payload'].get_records().get('Payload').decode('utf-8')

                        if not records.strip():
                            print("No data records found in the response.")
                        else:
                            master_gdf = pd.read_csv(StringIO(records))

                            chunksize = 10000
                            query_expression = f"FID == '{cf_reach_id}' and Stage == '{cf_control_stage}' and Extent == '{cf_flow}'"
                            #FAILED master_gdf = query_parquet_in_chunks(ripple_local_pathX, chunksize, query_expression)
                            #KEEP1
                            cnt = len(master_gdf)
                            #print(f"cnt =  {cnt}")
                            ...
                            if cnt > 0:
                                ...
                                #df_prj.columns = df_prj.columns.str.lower() #NEED TO DO THIS
                                #THIS IS A HACK FOR parquet hack test
                                df_prj.rename(columns={'FID': 'featureID'}, inplace=True)
                                df_prj.rename(columns={'Extent': 'flows'}, inplace=True)
                                df_prj.rename(columns={'Stage': 'stage'}, inplace=True)
                                df_prj['featureID'] = df_prj['featureID'].astype(str)
                                df_prj['flows'] = df_prj['flows'].astype(str)
                                df_prj['stage'] = df_prj['stage'].astype(str)
                                df_prj.columns = df_prj.columns.str.lower() #NEED TO DO THIS for postgres
                                ...
                                try:
                                    ...
                                    traceback_str = traceback.format_exc()
                                    logger.debug(f"[Step:Y-Y-Y] failed to create parquet file ")
                                    logger.debug(f"{traceback_str} ")

                                del df_prj #Clean up

                            del master_gdf #Clean up
                    ...
                    traceback_str = traceback.format_exc()
                    #logger.debug(f"[Step:X-X-X] failed to create parquet file ")
                    logger.debug(f"{traceback_str} ")


        #KEEP del df #Clean up
...
##############################################
        ...
        if False:
            try:
                ...
                #logger.debug(f"master_gdf columns {master_gdf.columns.tolist()}")
                #from osgeo import gdal
                #print(gdal.__version__)
                print(rasterio.__version__) #1.4.3
                print(gpd.__version__) #1.0.1
                print(np.__version__) #2.2.3
                #master_gdf.info(verbose=True)
    # import rasterio
    # import geopandas as gpd
    # from shapely.geometry import Polygon
    # import numpy as np
    ...
    # import unicodedata
                # type_mapping = {
                #     'int64': lambda col_name: f'int_{col_name}',
                #     'object': lambda col_name: f'str_{col_name}',
                #     'float64': lambda col_name: f'float_{col_name}'
                # }
                # master_gdf_renamed = rename_columns_by_type(master_gdf.copy(), type_mapping)


                #Create Parquet file for evaluation
                local_parquet_file = r'/tmp/OnTheFlyTest.parquet'
                #master_gdf.columns = master_gdf.columns.astype(str) #HACK FOR THIS TEST
                #master_gdf.to_parquet(local_parquet_file)
                #master_gdf_renamed.to_parquet(local_parquet_file, engine='fastparquet')
                master_gdf.to_parquet(local_parquet_file)

                output_parquet_bucket = output_log_bucket
                output_parquet_file = 'dev_temp/LorneLeonard/03-04-25/OnTheFlyTest.parquet'

                #s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/03-04-25/
                with open(local_parquet_file, "rb") as f:
                    s3_client.upload_fileobj(f, output_parquet_bucket, output_parquet_file)

            except Exception as e:
                traceback_str = traceback.format_exc()
                logger.debug(f"[Step:4-0-1] failed to create parquet file ")
                logger.debug(f"{traceback_str} ")

            try:
                #Create GPKG file for evaluation
                local_gpkg_file = r'/tmp/OnTheFlyTest.gpkg'
                master_gdf.to_file(local_gpkg_file, driver='GPKG', layer='name')
                output_gpkg_bucket = output_log_bucket
                output_gpkg_file = 'dev_temp/LorneLeonard/03-04-25/OnTheFlyTest.gpkg'

                #s3://hv-vpp-dev-ripple/dev_temp/LorneLeonard/03-04-25/
                with open(local_gpkg_file, "rb") as f:
                    s3_client.upload_fileobj(f, output_gpkg_bucket, output_gpkg_file)

            except Exception as e:
                traceback_str = traceback.format_exc()
                logger.debug(f"[Step:4-0-1] failed to create gpkg file ")
                logger.debug(f"{traceback_str} ")
    ...
    end_time = datetime.now()
    time_difference = end_time - start_time
    print(end_time.strftime("%Y-%m-%d %H:%M:%S")) # Output: 2025-02-03 16:40:01
    ...
    #TODO Discuss with group on meaningful return value for debugging
    result = {
        "statusCode": 200,  # Replace with the desired status code
