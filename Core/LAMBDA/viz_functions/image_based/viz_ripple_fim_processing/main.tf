...
locals {
  viz_service_name = "viz-ripple-fim-processing"
...
data "archive_file" "viz_service_zip" {
  ...
  source {
    content = templatefile("${path.module}/serverless.yml.tmpl", {
      ...
      # vvvvvvvv ENVIRONMENT VARIABLES vvvvvvvvvv
...
resource "time_sleep" "wait_for_build_finish" {
  triggers = {
    function_update = null_resource.start_build.triggers.source_hash
  }
  depends_on = [null_resource.start_build]
...
data "aws_lambda_function" "viz_lambda_function" {
  ...
  depends_on = [
    time_sleep.wait_for_build_finish
  ]
}