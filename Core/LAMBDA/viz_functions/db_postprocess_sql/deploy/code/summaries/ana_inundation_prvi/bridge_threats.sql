...
DROP TABLE IF EXISTS publish.ana_inundation_prvi_bridge_threats;
...
FROM cache.max_flows_ana_prvi AS flows
...
select mc.*
INTO publish.ana_inundation_prvi_bridge_threats
FROM (
	select osmid, 
...
ALTER TABLE publish.ana_inundation_prvi_bridge_threats
DROP COLUMN oid;
