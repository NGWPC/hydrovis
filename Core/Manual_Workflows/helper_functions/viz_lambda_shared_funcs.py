...
def get_db_engine(db_type):
    ...
    db_engine = create_engine(get_db_connection_url(db_type))
...
def run_sql_in_db(sql, db_type="viz", geodataframe=False):
    db_connection = get_db_connection(db_type)

    if geodataframe:
        import geopandas as gpd
        return gpd.read_postgis(sql, db_connection)
    else:
        import pandas as pd
        return pd.read_sql(sql, db_connection)

def run_sql_file_in_db(sql_file, db_type):
    print("Getting connection to run sql files")
    sql = open(sql_file, 'r').read()
    print(f"Running {sql_file}")
    run_sql_in_db(sql, db_type)
